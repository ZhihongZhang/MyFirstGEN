...
h=function(a){
	...
	layer.index++,
	b.index=layer.index,
	b.config=d.extend({},c,a),
	b.config.dialog=d.extend({},c.dialog,a.dialog),
	b.config.page=d.extend({},c.page,a.page),
	b.config.iframe=d.extend({},c.iframe,a.iframe),
	b.config.loading=d.extend({},c.loading,a.loading),
	b.config.tips=d.extend({},c.tips,a.tips),b.creat()},
	h.pt=h.prototype,
	h.pt.config={
		type:0,
		shade:[.3,"#000"],
	...
	h.pt.space=function(a){
		...
		return a=a||"",
				...
				h=[
					'<div class="xubox_dialog">'+f+'<span class="xubox_msg xubox_text" style="'+(f?"":"padding-left:20px")+'">'+e.msg+"</span></div>",
					...
					'<span class="xubox_loading xubox_loading_'+d.loading.type+'"></span>','<div class="xubox_tips" style="'+d.tips.style[0]+'"><div class="xubox_tipsMsg">'+d.tips.msg+'</div><i class="layerTipsG"></i></div>'
				],
				i="",
				j="",
				k=d.zIndex+c,
				...
				[
					i,
					'<div times="'+c+'" showtime="'+d.time+'" style="z-index:'+k+'" id="'+g[0]+c+'" class="'+g[0]+'">'+'<div style="background-color:'+d.bgcolor+"; z-index:"+k+'" class="xubox_main">'+h[d.type]+m+'<span class="xubox_setwin">'+n+"</span>"+'<span class="xubox_botton"></span>'+"</div>"+j+"</div>"
				]
	},
	h.pt.creat=function(){
		var k,l,m,a=this,
		...
		switch(c.title===!1?c.title=[]:"string"==typeof c.title&&(c.title=[c.title,!0]),c.type){
			...
			case 1:
				if(""!==h.html)
					j('<div class="xuboxPageHtml">'+h.html+"</div>"),
					i.append(d(b[1]));
				else if(""!==h.url)
					j('<div class="xuboxPageHtml" id="xuboxPageHtml'+f+'">'+h.html+"</div>"),
					i.append(d(b[1])),
					d.get(h.url,function(a){
						d("#xuboxPageHtml"+f).html(a.toString()),
						h.ok&&h.ok(a)
					});
				else{
					if(0!=d(h.dom).parents(g[4]).length)
						return;
						j(),
		...
		if(1!==c.type&&(j(),i.append(d(b[1]))),
			...
			c.title[1]&&(3!==c.type||4!==c.type)
		)
		switch(l=0===c.type?e:c,m=k.find(".xubox_botton"),
			l.btn=c.btn||e.btn,l.btns){
		case 0:
			m.html("").hide();
			break;
		case 1:
			...
			break;
		case 2:
			m.html('<a href="javascript:;" class="xubox_yes xubox_botton2">'+l.btn[0]+"</a>"+'<a href="javascript:;" class="xubox_no xubox_botton3">'+l.btn[1]+"</a>")}"auto"===k.css("left")?(k.hide(),
																																																setTimeout(function(){
																																																	k.show(),
																																																	a.set(f)
																																																},
	...
	h.pt.set=function(a){
		...
		switch(c.dialog,i=c.page,c.loading,k=b.layerE,l=k.find(g[2]),b.autoArea(a),c.title[1]?0===c.type&&layer.ie6&&l.css({width:k.outerWidth()}):4!==c.type&&k.find(".xubox_close").addClass("xubox_close1"),k.attr({type:f.type[c.type]}),b.offset(),4!==c.type&&(c.shift&&!layer.ie6?"object"==typeof c.shift?b.shift(c.shift[0],c.shift[1]||500,c.shift[2]):b.shift(c.shift,500):k.css({top:b.offsetTop,left:b.offsetLeft})),c.type){
			case 0:
				k.find(g[5]).css({"background-color":"#fff"}),
				c.title[1]?k.find(g[3]).css({paddingTop:18+l.outerHeight()}):(k.find(".xubox_msgico").css({top:8}),
				k.find(g[3]).css({marginTop:11}));
				break;
			case 1:
				k.find(i.dom).addClass("layer_pageContent"),
				c.shade[0]&&k.css({zIndex:c.zIndex+1}),
				c.title[1]&&k.find(g[4]).css({top:l.outerHeight()});
				break;
			case 2:
				m=k.find("."+g[1]),
				n=k.height(),
				m.addClass("xubox_load").css({width:k.width()}),
				c.title[1]?m.css({top:l.height(),height:n-l.height()}):m.css({top:0,height:n}),
			...
			case 4:
				...
				q={
					...
					top:p.offset().top,left:p.offset().left
				},
				r=k.find(".layerTipsG"),
				...
				q.autoLeft=function(){
					...
					q.where=[
						function(){
							...
							r.removeClass("layerTipsB").addClass("layerTipsT").css({"border-right-color":q.tipColor})
						},
						function(){
							q.tipLeft=q.left+q.width+10,
							q.tipTop=q.top,
							r.removeClass("layerTipsL").addClass("layerTipsR").css({"border-bottom-color":q.tipColor})
						},
						function(){
							q.autoLeft(),
							q.tipTop=q.top+q.height+10,
							r.removeClass("layerTipsT").addClass("layerTipsB").css({"border-right-color":q.tipColor})
						},
						function(){
							q.tipLeft=q.left-o[0]+10,
							q.tipTop=q.top,
							r.removeClass("layerTipsR").addClass("layerTipsL").css({"border-bottom-color":q.tipColor})
						}],
					q.where[c.tips.guide](),0===c.tips.guide?q.top-(e.scrollTop()+o[1]+16)<0&&q.where[2]():1===c.tips.guide?e.width()-(q.left+q.width+o[0]+16)>0||q.where[3]():2===c.tips.guide?q.top-e.scrollTop()+q.height+o[1]+16-e.height()>0&&q.where[0]():3===c.tips.guide?o[0]+16-q.left>0&&q.where[1]():4===c.tips.guide,k.css({left:q.tipLeft,top:q.tipTop})}c.fadeIn&&(f.fade(k,c.fadeIn,1),f.fade(d("#xubox_shade"+a),c.fadeIn,c.shade[0])),c.fix&&""===c.offset[0]&&!c.shift&&e.on("resize",
						function(){
							k.css({
				...
				h.pt.autoArea=function(a){
					...
					switch(a=a||b.index,c=b.config,e=c.page,f=d("#"+g[0]+a),h=f.find(g[2]),i=f.find(g[5]),j=c.title[1]?h.innerHeight():0,l=0,"auto"===c.area[0]&&i.outerWidth()>=c.maxWidth&&f.css({width:c.maxWidth}),c.type){
						...
						case 1:
							...
							"auto"===c.area[0]&&f.css({width:n.outerWidth()}),(""!==e.html||""!==e.url)&&(k=n.outerHeight());
							break;
						case 2:
